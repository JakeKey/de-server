...
router.post('/', async (req, res) => {
  ...
  if (products.length !== req.body.products.length ) return res.status(400).send('Invalid product');

  const productsWithQuantities = req.body.products.map((prod, i) => ({
    product: { _id: products[i]._id, name: products[i].name  }, quantity: prod.quantity 
  }));
